...
def build_model(input_img):
  if args.verbose: print('\nBUILDING VGG-19 NETWORK', flush=True)
  net = {}
  _, h, w, d     = input_img.shape
  
  if args.verbose: print('loading model weights...', flush=True)
  vgg_rawnet     = scipy.io.loadmat(args.model_weights)
  vgg_layers     = vgg_rawnet['layers'][0]
  if args.verbose: print('constructing layers...', flush=True)
  net['input']   = tf.Variable(np.zeros((1, h, w, d), dtype=np.float32))

  if args.verbose: print('LAYER GROUP 1', flush=True)
  ...
  if args.verbose: print('LAYER GROUP 2', flush=True)
  ...
  if args.verbose: print('LAYER GROUP 3', flush=True)
  ...
  if args.verbose: print('LAYER GROUP 4', flush=True)
  ...
  if args.verbose: print('LAYER GROUP 5', flush=True)
...
def conv_layer(layer_name, layer_input, W):
  ...
  if args.verbose: print('--{} | shape={} | weights_shape={}'.format(layer_name, 
    conv.get_shape(), W.get_shape()), flush=True)
  return conv
...
def pool_layer(layer_name, layer_input):
  if args.pooling_type == 'avg':
    ...
    pool = tf.nn.max_pool(layer_input, ksize=[1, 2, 2, 1], 
      strides=[1, 2, 2, 1], padding='SAME')
  if args.verbose: 
    print('--{}   | shape={}'.format(layer_name, pool.get_shape()), flush=True)
  return pool
...
def render_single_image():
  ...
  with tf.Graph().as_default():
    ...
    tock = time.time()
    print('Single image elapsed time: {}'.format(tock - tick), flush=True)
